...
class PabellonOcpList extends Component {
  ...
  componentDidMount() {
    pabellonService.getByState({"estado":"Ocupado"}).then((response) => {
      this.setState({
    ...
    return (
      <Container fluid className="main-content-container px-4">
        ...
        <Row noGutters className="page-header py-4">
          <PageTitle sm="4" title="Lista de Pabellones Ocupados" subtitle="" className="text-sm-left" />
        </Row>
              ...
              <Col lg="2" key={pabellon.id}>
                <Card small className="card-post mb-4">
                  <CardBody>
                    ...
                    <p className="card-text text-muted">Fecha y Hora Ingreso: {pabellon.hora}</p>
...
export default PabellonOcpList;
